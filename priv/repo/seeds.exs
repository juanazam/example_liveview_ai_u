...
countries = ["USA", "Canada", "Brazil", "Uruguay", "UK", "Germany", "France", "Australia", "Japan", "Singapore"]
...
names = [
  ...
  "Paul Rodriguez", "Quinn Thompson", "Rita Singh", "Sam Jackson", "Tina Liu",
  "Uma Johnson", "Victor Chang", "Wendy Zhang", "Xander Silva", "Yuki Tanaka",
  "Zoe Foster", "Aaron Brooks", "Bella Cooper", "Charlie Reed", "Diana Bell",
  "Ethan Gray", "Fiona Murphy", "Gabriel Ward", "Hanna Price", "Ivan Ross",
  "Julia Perry", "Kevin Powell", "Luna Cox", "Mason Hughes", "Nina Wells"
]
...
edge_cases = [
  %{
    ...
    total_spend: 50000,
    orders_count: 25,
    last_order_at: DateTime.add(base_time, -120 * 24 * 60 * 60, :second) |> DateTime.truncate(:second),
    signed_up_at: DateTime.add(base_time, -400 * 24 * 60 * 60, :second) |> DateTime.truncate(:second),
    country: "USA",
    segment: :enterprise,
  ...
  %{
    ...
    total_spend: 50,
    orders_count: 1,
    last_order_at: DateTime.add(base_time, -300 * 24 * 60 * 60, :second) |> DateTime.truncate(:second),
    signed_up_at: DateTime.add(base_time, -350 * 24 * 60 * 60, :second) |> DateTime.truncate(:second),
    country: "Canada",
    segment: :consumer,
  ...
  %{
    ...
    orders_count: 0,
    last_order_at: nil,
    signed_up_at: DateTime.add(base_time, -5 * 24 * 60 * 60, :second) |> DateTime.truncate(:second),
    country: "Brazil",
    segment: :consumer,
    has_open_support_ticket: false
  },
  %{
    ...
    total_spend: 150000,
    orders_count: 45,
    last_order_at: DateTime.add(base_time, -7 * 24 * 60 * 60, :second) |> DateTime.truncate(:second),
    signed_up_at: DateTime.add(base_time, -500 * 24 * 60 * 60, :second) |> DateTime.truncate(:second),
    country: "Germany",
    segment: :enterprise,
    has_open_support_ticket: false
  },
  %{
    ...
    total_spend: 75000,
    orders_count: 30,
    last_order_at: DateTime.add(base_time, -2 * 24 * 60 * 60, :second) |> DateTime.truncate(:second),
    signed_up_at: DateTime.add(base_time, -200 * 24 * 60 * 60, :second) |> DateTime.truncate(:second),
    country: "Singapore",
    segment: :enterprise,
    has_open_support_ticket: true
  },
  %{
    ...
    total_spend: 2500,
    orders_count: 8,
    last_order_at: DateTime.add(base_time, -45 * 24 * 60 * 60, :second) |> DateTime.truncate(:second),
    signed_up_at: DateTime.add(base_time, -180 * 24 * 60 * 60, :second) |> DateTime.truncate(:second),
    country: "Uruguay",
    segment: :smb,
    has_open_support_ticket: true
  }
...
for i <- 1..120 do
  ...
  orders_count = case segment do
    :enterprise -> :rand.uniform(50) + 10
    :smb -> :rand.uniform(20) + 2
    :consumer -> :rand.uniform(10)
  end
  
  total_spend = case {segment, status} do
    {:enterprise, :vip} -> :rand.uniform(100000) + 50000
  ...
  signed_up_at = DateTime.add(base_time, -signed_up_days_ago * 24 * 60 * 60, :second) |> DateTime.truncate(:second)
  
  last_order_at = case {status, orders_count} do
    {_, 0} -> nil
    {:active, _} -> 
